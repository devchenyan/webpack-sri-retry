    ...
    this.options = Object.assign(
      {
        ...
        JS_RETRY_SUCC_MSID: "",
        JS_RETRY_FAIL_MSID: 201,
        CSS_RETRY_SUCC_MSID: "",
        CSS_RETRY_FAIL_MSID: 202,
      },
      options
    );
  }
...
var report = function(data){
  setTimeout(function(){
    ...
    const { ext = {} } = data
    if (ext.msid > 200) {
      BJ_REPORT.onRetryError && BJ_REPORT.onRetryError(ext)
    } else if (ext.msid > 100) {
      BJ_REPORT.onFirstError && BJ_REPORT.onFirstError(ext)
    }
  },2000);
}
`;
